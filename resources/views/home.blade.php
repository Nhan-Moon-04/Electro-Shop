...
<!-- Hero Banner Slider -->
<section class="relative bg-gradient-to-r from-primary to-primary-700 text-white overflow-hidden" x-data="{ currentSlide: 0, slides: 3 }" x-init="setInterval(() => { currentSlide = (currentSlide + 1) % slides }, 5000)">
    <div class="container mx-auto px-4 py-16 md:py-24">
        <!-- Slide 1 -->
        <div x-show="currentSlide === 0" x-transition class="grid md:grid-cols-2 gap-8 items-center">
            <div>
                <span class="inline-block bg-yellow-400 text-gray-900 px-4 py-1 rounded-full text-sm font-bold mb-4">KHUYẾN MÃI HOT</span>
                <h1 class="text-4xl md:text-6xl font-bold mb-4">Giảm đến 50%</h1>
                <p class="text-xl mb-6">Cho tất cả sản phẩm điện tử cao cấp</p>
                <a href="{{ route('products.index') }}" class="inline-block bg-white text-primary px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition">
        ...
        <div x-show="currentSlide === 1" x-transition class="grid md:grid-cols-2 gap-8 items-center">
            <div>
                <span class="inline-block bg-red-500 text-white px-4 py-1 rounded-full text-sm font-bold mb-4">FLASH SALE</span>
        ...
        <div x-show="currentSlide === 2" x-transition class="grid md:grid-cols-2 gap-8 items-center">
            <div>
                <span class="inline-block bg-green-500 text-white px-4 py-1 rounded-full text-sm font-bold mb-4">MỚI VỀ</span>
    ...
    <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2">
        <button @click="currentSlide = 0" :class="currentSlide === 0 ? 'bg-white' : 'bg-white/50'" class="w-3 h-3 rounded-full transition"></button>
        <button @click="currentSlide = 1" :class="currentSlide === 1 ? 'bg-white' : 'bg-white/50'" class="w-3 h-3 rounded-full transition"></button>
        <button @click="currentSlide = 2" :class="currentSlide === 2 ? 'bg-white' : 'bg-white/50'" class="w-3 h-3 rounded-full transition"></button>
    </div>
</section>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            ...
            <a href="{{ route('products.category', $category->category_id) }}" class="group">
                <div class="card p-6 text-center hover:scale-105 transition-all duration-300">
                    <div class="bg-primary-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition">
                        @if($category->category_img)
                            <img src="{{ asset('imgs/categories/' . $category->category_img) }}" alt="{{ $category->category_name }}" class="w-12 h-12 object-contain">
                        @else
                            <i class="fas fa-desktop text-3xl text-primary"></i>
                        @endif
                    </div>
                    <h3 class="font-semibold text-gray-800 group-hover:text-primary transition">{{ $category->category_name }}</h3>
                </div>
            </a>
            @endforeach
        </div>
...
<section class="py-12 bg-gradient-to-r from-red-500 to-orange-500">
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-8">
            ...
            <div class="hidden md:flex items-center space-x-2 bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2 text-white">
...
@endif
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <span class="text-gray-500 text-sm ml-2">({{ number_format($product->product_rate ?? 0, 1) }})</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                            @if($product->min_price)
                                <p class="text-red-600 font-bold text-lg">{{ number_format($product->min_price, 0, ',', '.') }}₫</p>
                            @else
                                <p class="text-red-600 font-bold text-lg">Liên hệ</p>
                            @endif
                        </div>
                        <button class="bg-primary text-white w-10 h-10 rounded-lg hover:bg-primary-600 transition" onclick="event.stopPropagation();">
        ...
        <div class="text-center mt-8">
            <a href="{{ route('products.index') }}" class="inline-block bg-white text-red-500 px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition">
        ...
        <div class="grid grid-cols-3 md:grid-cols-6 gap-8 items-center">
            ...
            <div class="grayscale hover:grayscale-0 transition cursor-pointer">
                @if($supplier->supplier_logo)
                    <img src="{{ asset('imgs/logo-brand/' . $supplier->supplier_logo) }}" alt="{{ $supplier->supplier_name }}" class="w-full h-20 object-contain">
                @else
...
<section class="py-12 bg-gradient-to-r from-primary to-primary-700 text-white">
    <div class="container mx-auto px-4 text-center">
        ...
        <form class="max-w-md mx-auto flex gap-4">
            <input type="email" placeholder="Nhập email của bạn" class="flex-1 px-4 py-3 rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-white">
            <button type="submit" class="bg-white text-primary px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition">
                Đăng ký
            </button>
        </form>
    </div>
